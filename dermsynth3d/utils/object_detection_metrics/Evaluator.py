...
from dermsynth3d.utils.object_detection_metrics.BoundingBox import *
...
class Evaluator:
    def GetDetection(
        ...
        for c in classes:
            ...
            det2 = Counter([cc[0] for cc in gts])
            for key, val in det.items():
                det[key] = np.zeros(val)
            for key, val in det2.items():
                det2[key] = np.zeros(val)
            ...
            for d in range(len(dects)):
                ...
                if jmax is not None and det2[dects[d][0]][jmax] == 0:
                    IoU[d] = iouMax
